/*
	...
	Copyright:	Copyright © 1995-1996 Jim Luther.  All Rights Reserved.

	Change History (most recent first):

         <2>      6/5/98    rmgw    Fix iterator filter.
*/
		 ...
		 (theGlobals->currentLevel < theGlobals->maxLevels) )	/* if currentLevel < maxLevels, look at this level */
	{
		...
		{	/* Isn't C great... What I'd give for a "WITH theGlobals DO" about now... */
			...
			if ( theGlobals->result == noErr )
			{
				...
				CallIterateFilterProc(theGlobals->iterateFilter, &theGlobals->cPB, dirID, &theGlobals->quitFlag, theGlobals->yourDataPtr);
