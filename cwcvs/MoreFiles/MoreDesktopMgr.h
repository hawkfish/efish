...
/*	Ï Open a volume's desktop database and return the desktop database refNum.
...
pascal	OSErr	DTXGetAPPL(ConstStr255Param volName,
						   ...
						   long *applParID,
						   Str255 applName);
/*	Ï Find an application on a volume that can open a file with a given creator.
...
pascal	OSErr	DTGetIcon(ConstStr255Param volName,
						  ...
						  OSType fileType,
						  Handle *iconHandle);
/*	Ï Get an icon from the desktop database or Desktop file.
...
/*	Ï Set a file or directory's Finder comment field.
	...
	Result Codes
		...
		nsvErr				-35		Volume not found
		ioErr				-36		I/O error
		fnfErr				Ü43		File or directory doesnÍt exist
		paramErr			-50		Volume doesn't support this function
		wPrErr				Ü44		Volume is locked through hardware
		vLckdErr			Ü46		Volume is locked through software
		rfNumErr			Ü51		Reference number invalid
		extFSErr			-58		External file system error - no file
									system claimed this call.
		desktopDamagedErr	-1305	The desktop database has become corrupted - 
									the Finder will fix this, but if your
...
pascal	OSErr	DTGetComment(short vRefNum,
							 ...
							 ConstStr255Param name,
							 Str255 comment);
/*	Ï Get a file or directory's Finder comment field (if any).
...
/*	Ï Copy the file or folder comment from the source to the destination object.
...
/*	Ï Copy the desktop database comment from the source to the destination object.
