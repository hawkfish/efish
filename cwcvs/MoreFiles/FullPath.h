...
/*
	...
	In general, you should use a fileÍs name, parent directory ID, and
	...
	can save the fileÍs name, its parent directory ID, and the name of the
	volume on which itÍs located. Although none of these methods is
	foolproof, they are much more reliable than using full pathnames to
	identify files.
	
	Nonetheless, it is sometimes useful to display a fileÍs full pathname
	...
	a file when it needs the userÍs confirmation to delete the file. No
...
pascal	OSErr	GetFullPath(short vRefNum,
							...
							Handle *fullPath);
/*	Ï Get a full pathname to a volume, directory or file.
...
pascal OSErr FSpLocationFromFullPath(short fullPathLength,
									 const void *fullPath,
									 FSSpec *spec);
/*	Ï Get a FSSpec from a full pathname.
...
pascal OSErr LocationFromFullPath(short fullPathLength,
								  ...
								  long *parID,
								  Str31 name);
/*	Ï Get an object's location from a full pathname.
