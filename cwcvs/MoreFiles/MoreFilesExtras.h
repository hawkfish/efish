...
pascal	void	TruncPString(StringPtr destination,
							 ...
							 short maxLength);
/*	Ï International friendly string truncate routine.
...
pascal	Ptr	GetTempBuffer(long buffReqSize,
						  long *buffActSize);
/*	Ï Allocate a temporary copy or search buffer.
...
pascal	OSErr	GetVolumeInfoNoName(ConstStr255Param pathname,
									short vRefNum,
									HParmBlkPtr pb);
/*	Ï Call PBHGetVInfoSync ignoring returned name.
...
pascal	OSErr GetCatInfoNoName(short vRefNum,
							   ...
							   CInfoPBPtr pb);
/*	Ï Call PBGetCatInfoSync ignoring returned name.
...
pascal	OSErr	DetermineVRefNum(ConstStr255Param pathname,
								 short vRefNum,
								 short *realVRefNum);
/*	Ï Determine the real volume reference number.
...
/*	Ï Get information about a mounted volume.
...
/*	Ï Get extended information about a mounted volume.
...
pascal	OSErr	CheckVolLock(ConstStr255Param pathname,
							 short vRefNum);
/*	Ï Determine if a volume is locked.
...
/*	Ï Find a volume's drive queue element in the drive queue.
...
/*	Ï Return the number of physical disk blocks on a disk drive.
	...
	Result Codes
		...
		nsDrvErr			-56		No such drive
		statusErr			Ü18		Driver does not respond to this
									status request
		badUnitErr			Ü21		Driver reference number does not
									match unit table
		unitEmptyErr		Ü22		Driver reference number specifies
									a nil handle in unit table
		abortErr			Ü27		Request aborted by KillIO
		notOpenErr			Ü28		Driver not open
*/
...
pascal	OSErr	GetVolFileSystemID(ConstStr255Param pathname,
								   short vRefNum,
								   short *fileSystemID);
/*	Ï Get a volume's file system ID.
...
/*	Ï Returns a volume's online and eject information.
...
pascal	OSErr	OnLine(FSSpecPtr volumes,
					   ...
					   short *actVolCount,
					   short *volIndex);
/*	Ï Return the list of volumes currently mounted.
...
pascal	OSErr SetDefault(short newVRefNum,
						 ...
						 short *oldVRefNum,
						 long *oldDirID);
/*	Ï Set the default volume before making Standard I/O requests.
...
pascal	OSErr RestoreDefault(short oldVRefNum,
							 long oldDirID);
/*	Ï Restore the default volume after making Standard C I/O requests.
...
pascal	OSErr GetDInfo(short vRefNum,
					   ...
					   ConstStr255Param name,
					   DInfo *fndrInfo);
/*	Ï Get the finder information for a directory.
...
/*	Ï Set the finder information for a directory.
...
pascal	OSErr	GetDirectoryID(short vRefNum,
							   ...
							   long *theDirID,
							   Boolean *isDirectory);
/*	Ï Get the directory ID number of the directory specified.
...
pascal	OSErr	FSpGetDirectoryID(const FSSpec *spec,
								  long *theDirID,
								  Boolean *isDirectory);
/*	Ï Get the directory ID number of a directory.
...
pascal	OSErr	GetDirName(short vRefNum,
						   long dirID,
						   Str31 name);
/*	Ï Get the name of a directory from its directory ID.
...
pascal	OSErr	GetIOACUser(short vRefNum,
							...
							ConstStr255Param name,
							SInt8 *ioACUser);
/*	Ï Get a directory's access restrictions byte.
...
pascal	OSErr	GetParentID(short vRefNum,
							...
							ConstStr255Param name,
							long *parID);
/*	Ï Get the parent directory ID number of the specified object.
...
pascal	OSErr	GetFilenameFromPathname(ConstStr255Param pathname,
										Str255 filename);
/*	Ï Get the object name from the end of a full or partial pathname.
...
pascal	OSErr	GetObjectLocation(short vRefNum,
								  ...
								  Str255 realName,
								  Boolean *isDirectory);
/*	Ï Get a file system object's location.
...
pascal	OSErr	GetDirItems(short vRefNum,
							...
							short *actItemCount,
							short *itemIndex);
/*	Ï Return a list of items in a directory.
...
pascal	OSErr	DeleteDirectoryContents(short vRefNum,
								 		long dirID,
										ConstStr255Param name);
/*	Ï Delete the contents of a directory.
...
pascal	OSErr	DeleteDirectory(short vRefNum,
								long dirID,
								ConstStr255Param name);
/*	Ï Delete a directory and its contents.
...
pascal	OSErr	CheckObjectLock(short vRefNum,
								long dirID,
								ConstStr255Param name);
/*	Ï Determine if a file or directory is locked.
...
pascal	OSErr	GetFileSize(short vRefNum,
							...
							long *dataSize,
							long *rsrcSize);
/*	Ï Get the logical sizes of a file's forks.
...
pascal	OSErr	BumpDate(short vRefNum,
						 long dirID,
						 ConstStr255Param name);
/*	Ï Update the modification date of a file or directory.
...
pascal	OSErr	ChangeCreatorType(short vRefNum,
								  ...
								  OSType creator,
								  OSType fileType);
/*	Ï Change the creator or file type of a file.
...
/*	Ï Set or clear Finder Flag bits.
...
pascal	OSErr	SetIsInvisible(short vRefNum,
							   long dirID,
							   ConstStr255Param name);
/*	Ï Set the invisible Finder Flag bit.
...
pascal	OSErr	ClearIsInvisible(short vRefNum,
								 long dirID,
								 ConstStr255Param name);
/*	Ï Clear the invisible Finder Flag bit.
...
pascal	OSErr	SetNameLocked(short vRefNum,
							  long dirID,
							  ConstStr255Param name);
/*	Ï Set the nameLocked Finder Flag bit.
...
pascal	OSErr	ClearNameLocked(short vRefNum,
								long dirID,
								ConstStr255Param name);
/*	Ï Clear the nameLocked Finder Flag bit.
...
pascal	OSErr	SetIsStationery(short vRefNum,
								long dirID,
								ConstStr255Param name);
/*	Ï Set the isStationery Finder Flag bit.
...
pascal	OSErr	ClearIsStationery(short vRefNum,
								  long dirID,
								  ConstStr255Param name);
/*	Ï Clear the isStationery Finder Flag bit.
...
pascal	OSErr	SetHasCustomIcon(short vRefNum,
								 long dirID,
								 ConstStr255Param name);
/*	Ï Set the hasCustomIcon Finder Flag bit.
...
pascal	OSErr	ClearHasCustomIcon(short vRefNum,
								   long dirID,
								   ConstStr255Param name);
/*	Ï Clear the hasCustomIcon Finder Flag bit.
...
pascal	OSErr	ClearHasBeenInited(short vRefNum,
								   long dirID,
								   ConstStr255Param name);
/*	Ï Clear the hasBeenInited Finder Flag bit.
...
pascal	OSErr	CopyFileMgrAttributes(short srcVRefNum,
									  ...
									  Boolean copyLockBit);
/*	Ï Copy all File Manager attributes from the source to the destination.
...
pascal	OSErr	HOpenAware(short vRefNum,
						   ...
						   short denyModes,
						   short *refNum);
/*	Ï Open the data fork of a file using deny mode permissions.
...
pascal	OSErr	HOpenRFAware(short vRefNum,
							 ...
							 short denyModes,
							 short *refNum);
/*	Ï Open the resource fork of a file using deny mode permissions.
...
pascal	OSErr	FSReadNoCache(short refNum,
							  long *count,
							  void *buffPtr);
/*	Ï Read any number of bytes from an open file requesting no caching.
	...
	Result Codes
		noErr				0		No error
		readErr				Ü19		Driver does not respond to read requests
		badUnitErr			Ü21		Driver reference number does not
									match unit table
		unitEmptyErr		Ü22		Driver reference number specifies a
									nil handle in unit table
		abortErr			Ü27		Request aborted by KillIO
		notOpenErr			Ü28		Driver not open
		ioErr				Ü36		Data does not match in read-verify mode
...
pascal	OSErr	FSWriteNoCache(short refNum,
							   long *count,
							   const void *buffPtr);
/*	Ï Write any number of bytes to an open file requesting no caching.
	...
	Result Codes
		noErr				0		No error
		writErr				Ü20		Driver does not respond to write requests
		badUnitErr			Ü21		Driver reference number does not
									match unit table
		unitEmptyErr		Ü22		Driver reference number specifies a
									nil handle in unit table
		...
		vLckdErr			-46		Software volume lock	
		rfNumErr			-51		Bad reference number
		wrPermErr			-61		Read/write permission doesnÍt
									allow writing	
		afpAccessDenied		-5000	User does not have the correct access to
									the file
...
pascal	OSErr	FSWriteVerify(short refNum,
							  long *count,
							  const void *buffPtr);
/*	Ï Write any number of bytes to an open file and then verify the data was written.
...
pascal	OSErr	CopyFork(short srcRefNum,
						 ...
						 void *copyBufferPtr,
						 long copyBufferSize);
/*	Ï Copy all data from the source fork to the destination fork of open file forks.
...
pascal	OSErr	GetFileLocation(short refNum,
								...
								long *dirID,
								StringPtr fileName);
/*	Ï Get the location of an open file.
	...
	Result Codes
		noErr				0		No error
		nsvErr				-35		Specified volume doesnÍt exist
		fnOpnErr			-38		File not open
...
pascal	OSErr	FSpGetFileLocation(short refNum,
								   FSSpec *spec);
/*	Ï Get the location of an open file in an FSSpec record.
...
pascal	OSErr	CopyDirectoryAccess(short srcVRefNum,
									...
									long dstDirID,
									ConstStr255Param dstName);
/*	Ï Copy the AFP directory access privileges.
...
/*	Ï Move a file or directory and optionally rename it.
...
/*	Ï Initialize the fields of an AFPVolMountInfo record.
...
/*	Ï Retrieve the AFP mounting information from an AFPVolMountInfo record.
...
pascal	OSErr	GetUGEntries(short objType,
							 ...
							 long *actEntryCount,
							 long *objID);
/*	Ï Retrieve a list of user or group entries from the local file server.
