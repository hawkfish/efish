...
typedef struct FormatFlags
	...
	short	version;
	short	subVersion; 
	long	flags0;			/* Use PIModFlag here! */
	long	flags1;			/* reserved -- set to 0 for now */
	long	signature;
	}
FormatFlags;
...
typedef unsigned char PIAlphaType;	// this type is not really used, but you're welcome to
...
typedef unsigned long	PIAlphaFlags;

typedef struct {
	short					version;
	PIAlphaFlags			flags;
	unsigned char			red;			// color that was matted (if premul)
	unsigned char			green;
	unsigned char			blue;
	PIAlphaType				alpha;
...
typedef struct TimeInfo
	{
		long			time_info_version;								// hi word is major, lo is sub

		
		/* info returned from all selectors */
		
		char			error_msg[PI_FORMAT_T_MESSAGE_LEN+1];			// set to non-null string
		...
		short			time_base;					// 24, 25, 30, or -30 for dropframe
		char			reel_name[16];
		...
		long			frame_num_to_add;	// which frame is being added (-1 for next)
		long			frames_to_add;		// how many frames does this sample last?
		Boolean			was_compressed;		// data has already been compressed/decompressed 
		long			origin_h;			// ignored
		long			origin_v;			// ignored
...
typedef struct TimeExtension
	{
	long				time_selector;		/* 	this value is the FXIF plug-in selector.
...
typedef struct FormatRecordT
	...
	short				hostModes;		/* Used by the host to inform the plug-in which
