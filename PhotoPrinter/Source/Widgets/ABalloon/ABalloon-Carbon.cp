...
#include <LStream.h>
	...
	#error this implementation is only for Carbon.
#endif
#if !__option(RTTI)
...
/*	---------------------------------------------------------------------------------------------
	...
	History:
				...
				2.0b2 10/8/2000
					• Updated not to use the WrapHack for CarbonLib 1.1.
				2.0b3 11/2/2000
					• Changed the Carbon implementation to use balloons instead of
					  tags when running under CarbonLib 1.0.x.  I did this because
					  of the CarbonLib bug where tags appear behind modal dialogs.
					• Call LView::OutOfFocus( nil ) at the end of the window content
					  callback.  Without this change, my app's insertion point was
					  flashing within the help tag.
				2.0b4 11/17/2000
					• As suggested by Isaac Wankerl, conditionalized the code so
					  that it would compile with Universal Interfaces 3.3.2, which
					  lacked CarbonEvents.h.
				...
				2.0b5 12/10/2000
					...
					• Renamed CalcPortExposedFrame to CalcPortHotRect, and made it
					  a method of ABalloonHelper.
					• Removed the WrapHack, because I am using balloons for
					  CarbonLib 1.0 and the hack is not necessary in CarbonLib 1.1.
...
#pragma mark class ABalloonHelperTag
/*	---------------------------------------------------------------------------------------------
	class ABalloonHelperTag
	
	This version of ABalloonHelper is used when running under Mac OS X
	or with CarbonLib 1.1 or better, where we will use Carbon Help.
	---------------------------------------------------------------------------------------------
*/
class ABalloonHelperTag : public ABalloonHelper
{
public:
							ABalloonHelperTag();
					
	virtual					~ABalloonHelperTag();
	...
	static bool				IsControlKeyPressed();

	static void				SupplyContent(
									ABalloonBase* inABalloon,
									HMContentProvidedType *outContentProvided,
									HMHelpContentPtr ioHelpContent );
									
	static pascal OSStatus	WindowCallback(
									WindowRef inWindow,
									Point inGlobalMouse,
	...
	HMWindowContentUPP	mCallbackUPP;
	
#if UNIVERSAL_INTERFACES_VERSION >= 0x0335
	static pascal OSStatus	ModifierKeyHandler(
									EventHandlerCallRef inHandlerCallRef,
									EventRef inEvent,
									void *inUserData );

	EventHandlerUPP		mModifierEventUPP;
	EventHandlerRef		mModifierHandlerRef;
	bool				mHaveCarbonEvents;
#endif

	static	bool	sShortDelayMode;
};
...
public:
							ABalloonHelperBalloon(
									CFragConnectionID		inInterfaceLib );
	...
	pascal OSErr			(*HMShowBalloonProc)(
									...
									SInt16                  inMethod	);
	pascal Boolean			(*HMIsBalloonProc)( void );
	pascal Boolean			(*HMGetBalloonsProc)( void );
	pascal OSErr			(*HMSetBalloonsProc)( Boolean inFlag );
...
bool					ABalloonHelperTag::sShortDelayMode = false;
...
void	ABalloonHelper::Initialize()		// [static]
{
	if (sTheHelper == nil)
	{
		...
		if ( (err == noErr) && (carbonVersion < 0x00000110) )
		{
			CFragConnectionID	connectID;
			OSErr err = ::GetSharedLibrary( "\pInterfaceLib", kPowerPCCFragArch,
				kFindCFrag, &connectID, nil, nil );
			if ( err == noErr )
			{
				sTheHelper = NEW ABalloonHelperBalloon( connectID );
			}
			else	// maybe some DP version of Mac OS X
			{
				sTheHelper = NEW ABalloonHelperTag;
			}
		}
		else
		{
...
#pragma mark -
ABalloonHelperTag::ABalloonHelperTag()
	: mCallbackUPP( NewHMWindowContentUPP( ABalloonHelperTag::WindowCallback ) )
	#if UNIVERSAL_INTERFACES_VERSION >= 0x0335
		,mModifierEventUPP( nil ),
		mModifierHandlerRef( nil ),
		mHaveCarbonEvents( (long)::InstallEventHandler != kUnresolvedCFragSymbolAddress )
	#endif
{
	#if UNIVERSAL_INTERFACES_VERSION >= 0x0335
		if (mHaveCarbonEvents)
		{
			mModifierEventUPP = NewEventHandlerUPP( ModifierKeyHandler );
		}
	#endif
	if (sControlKeyPop)
	{
		StartWatchingControlKey();
	}
...
ABalloonHelperTag::~ABalloonHelperTag()
{
	if (sControlKeyPop)
	{
		StopWatchingControlKey();
	}
	sTheHelper = nil;
	DisposeHMWindowContentUPP( mCallbackUPP );
	
	#if UNIVERSAL_INTERFACES_VERSION >= 0x0335
		if (mModifierEventUPP != nil)
		{
			DisposeEventHandlerUPP( mModifierEventUPP );
		}
	#endif
}

void	ABalloonHelperTag::AddABalloon( ABalloonBase* inABalloon )
{
	ABalloonHelper::AddABalloon( inABalloon );
	
	// Make sure that the host's window has a content callback.
	LPane*	ownerPane = dynamic_cast<LPane*>( inABalloon->GetOwnerHost() );
	if (ownerPane != nil)
	{
		WindowRef	ownerWindow = ownerPane->GetMacWindow();
		if (ownerWindow != nil)
		{
			HMWindowContentUPP	callbackUPP;
			if ( (noErr == ::HMGetWindowContentCallback( ownerWindow, &callbackUPP )) &&
				(callbackUPP == nil) )
			{
				::HMInstallWindowContentCallback( ownerWindow, mCallbackUPP );
			}
		}
	}
}
...
/*	---------------------------------------------------------------------------------------------
	ABalloonHelperTag::SpendTime
	
	The helper is a repeater only when the control key feature is enabled.
	---------------------------------------------------------------------------------------------
*/
void	ABalloonHelperTag::SpendTime( const EventRecord	& /* inMacEvent */ )
{
	CheckControlKey();
}

/*	---------------------------------------------------------------------------------------------
	ABalloonHelperTag::SupplyContent			[static]
	---------------------------------------------------------------------------------------------
*/
void	ABalloonHelperTag::SupplyContent(
									ABalloonBase* inABalloon,
									HMContentProvidedType *outContentProvided,
									HMHelpContentPtr ioHelpContent )
{
	LPane*	myPane = dynamic_cast<LPane*>( inABalloon->GetOwnerHost() );
	
	// Compute the hot rectangle in global coordinates.
	Rect	myRect;
	CalcPortHotRect( myPane, myRect );
	if (!::EmptyRect( &myRect ))
	{
		myPane->PortToGlobalPoint( topLeft(myRect) );
		myPane->PortToGlobalPoint( botRight(myRect) );
		ioHelpContent->absHotRect = myRect;
		
		// Find the content string.
		Str255	theString;
		inABalloon->GetHelpString( myPane, theString );
		if (theString[0] != 0)
		{
			ioHelpContent->tagSide = kHMDefaultSide;
			ioHelpContent->content[1].contentType = kHMNoContent;
			ioHelpContent->content[0].contentType = kHMPascalStrContent;
			::BlockMoveData( theString, ioHelpContent->content[0].u.tagString,
				theString[0]+1 );
...
/*	---------------------------------------------------------------------------------------------
	ABalloonHelperTag::WindowCallback			[static]
	
	Carbon Help calls this when the mouse pauses in the window.
	---------------------------------------------------------------------------------------------
*/
pascal OSStatus		ABalloonHelperTag::WindowCallback(
									WindowRef inWindow,
									Point inGlobalMouse,
									HMContentRequest inRequest,
									HMContentProvidedType *outContentProvided,
									HMHelpContentPtr ioHelpContent )
{
	...
	*outContentProvided = kHMContentNotProvided;
	...
	if ((inRequest == kHMSupplyContent) && (sTheHelper != nil))
	{
		ABalloonBase*	theBalloon = sTheHelper->FindBalloonPane( inGlobalMouse,
			inWindow );
		if ( (theBalloon != nil) &&
			(
				sAutoPop ||
				theBalloon->GetAutomatic() ||
				(sControlKeyPop && IsControlKeyPressed())
			)
		)
		{
			SupplyContent( theBalloon, outContentProvided, ioHelpContent );
...
/*	---------------------------------------------------------------------------------------------
	ABalloonHelperTag::ModifierKeyHandler		[static]
	
	This Carbon Event handler is called when the state of the
	modifier keys changes.
	---------------------------------------------------------------------------------------------
*/
#if UNIVERSAL_INTERFACES_VERSION >= 0x0335
pascal OSStatus	ABalloonHelperTag::ModifierKeyHandler(
									EventHandlerCallRef /* inHandlerCallRef */,
									EventRef /* inEvent */,
									void * /* inUserData */ )
{
	static_cast<ABalloonHelperTag*>(sTheHelper)->CheckControlKey();
	return noErr;
}
#endif

/*	---------------------------------------------------------------------------------------------
	ABalloonHelperTag::IsControlKeyPressed		[static]
	---------------------------------------------------------------------------------------------
*/
bool	ABalloonHelperTag::IsControlKeyPressed()
{
	KeyMap			theKeyMap;
	::GetKeys( theKeyMap );
	return ((theKeyMap[1] & 0x0008) != 0);
}

void	ABalloonHelperTag::StartWatchingControlKey()
{
#if UNIVERSAL_INTERFACES_VERSION >= 0x0335
	if (mHaveCarbonEvents)
	{
		const EventTypeSpec	kModKeyEvent =
			{ kEventClassKeyboard, kEventRawKeyModifiersChanged };
		::InstallEventHandler( ::GetApplicationEventTarget(), mModifierEventUPP,
			1, &kModKeyEvent, nil, &mModifierHandlerRef );
	}
	else
#endif
	{
...
void	ABalloonHelperTag::StopWatchingControlKey()
{
#if UNIVERSAL_INTERFACES_VERSION >= 0x0335
	if (mHaveCarbonEvents)
	{
		::RemoveEventHandler( mModifierHandlerRef );
		mModifierHandlerRef = nil;
	}
	else
#endif
...
/*	---------------------------------------------------------------------------------------------
	ABalloonHelperTag::CheckControlKey

	Reduce the delay while the control key is pressed.
	---------------------------------------------------------------------------------------------
*/
void	ABalloonHelperTag::CheckControlKey()
{
	if (IsControlKeyPressed())
	{
		if (!sShortDelayMode)
		{
			sShortDelayMode = true;
			::HMSetTagDelay( 1 );
		}
	}
	else	// control key not pressed
	{
		if (sShortDelayMode)
		{
			sShortDelayMode = false;
			::HMSetTagDelay( sAutoPopTicks * kMillisecondsPerTick );
		}
	}
}
...
ABalloonHelperBalloon::ABalloonHelperBalloon( CFragConnectionID inInterfaceLib )
	: mLastBalloon( nil )
{
	StartRepeating();
	
	::FindSymbol( inInterfaceLib, "\pHMShowBalloon", (Ptr*)&HMShowBalloonProc, nil );
	::FindSymbol( inInterfaceLib, "\pHMIsBalloon", (Ptr*)&HMIsBalloonProc, nil );
	::FindSymbol( inInterfaceLib, "\pHMGetBalloons", (Ptr*)&HMGetBalloonsProc, nil );
	::FindSymbol( inInterfaceLib, "\pHMSetBalloons", (Ptr*)&HMSetBalloonsProc, nil );
}
...
void	ABalloonHelperBalloon::SpendTime( const EventRecord	&inMacEvent )
{
		...
		sOverBalloonPane )
	{
		...
		if (sControlKeyPop)
		{
			...
			{
				if ( !HMGetBalloonsProc() )
				{
					HMSetBalloonsProc(true);
		...
		if (theABalloon)
		{
			if (HMGetBalloonsProc())	// balloons turned on manually?
			{
									...
									const Rect&				inAltRect )
{
	...
	else
	{
		...
		HMShowBalloonProc( &theMessageRec,
			theTip,
...
void	ABalloonBase::SetAutoPopDelay( UInt32 inDelayTicks )
{
	...
	::HMSetTagDelay( ABalloonHelper::sAutoPopTicks * kMillisecondsPerTick );
	ABalloonHelperTag::sShortDelayMode = false;
}
