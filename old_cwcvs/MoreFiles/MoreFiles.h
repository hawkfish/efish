...
pascal	OSErr	HGetVolParms(ConstStr255Param volName,
							 ...
							 long *infoSize);
/*	Ï Determine the characteristics of a volume.
...
/*	Ï Create a new file with no creator or file type.
...
/*	Ï Exchange the data stored in two files on the same volume.
...
pascal	OSErr	ResolveFileIDRef(ConstStr255Param volName,
								 ...
								 long *parID,
								 StringPtr fileName);
/*	Ï Retrieve the location of the file with the specified file ID reference.
...
pascal	OSErr	CreateFileIDRef(short vRefNum,
								...
								ConstStr255Param fileName,
								long *fileID);
/*	Ï Establish a file ID reference for a file.
...
/*	Ï Write the contents of a file's access path buffer (the fork data).
...
pascal	OSErr	UnlockRange(short refNum,
							long rangeLength,
							long rangeStart);
/*	Ï Unlock a previously locked range.
...
/*	Ï Retrieve the native access-control information.
...
pascal	OSErr	SetForeignPrivs(short vRefNum,
								...
								long foreignPrivInfo3,
								long foreignPrivInfo4);
/*	Ï Change the native access-control information.
...
pascal	OSErr	HGetLogInInfo(ConstStr255Param volName,
							  ...
							  short *loginMethod,
							  StringPtr userName);
/*	Ï Get the login method and user name used to log on to a shared volume.
...
pascal	OSErr	HGetDirAccess(short vRefNum,
							  ...
							  long *groupID,
							  long *accessRights);
/*	Ï Get a directory's access control information on a shared volume.
...
pascal	OSErr	HSetDirAccess(short vRefNum,
							  ...
							  long groupID,
							  long accessRights);
/*	Ï Set a directory's access control information on a shared volume.
...
pascal	OSErr	HMapID(ConstStr255Param volName,
					   ...
					   short objType,
					   StringPtr name);
/*	Ï Map a user or group ID to a user or group name.
...
pascal	OSErr	HMapName(ConstStr255Param volName,
						 ...
						 short objType,
						 long *ugID);
/*	Ï Map a user or group name to a user or group ID.
...
/*	Ï Duplicate a file on a file server and optionally to rename it.
...
pascal	OSErr	HMoveRename(short vRefNum,
							...
							ConstStr255Param dstpathName,
							ConstStr255Param copyName);
/*	Ï Move a file or directory on a file server and optionally to rename it.
...
pascal	OSErr	GetVolMountInfoSize(ConstStr255Param volName,
									short vRefNum,
									short *size);
/*	Ï Get the size of a volume mounting information record.
...
pascal	OSErr	GetVolMountInfo(ConstStr255Param volName,
								short vRefNum,
								void *volMountInfo);
/*	Ï Retrieve a volume mounting information record.
...
pascal	OSErr	VolumeMount(const void *volMountInfo,
							short *vRefNum);
/*	Ï Mount a volume using a volume mounting information record.
...
pascal	OSErr	Share(short vRefNum,
					  long dirID,
					  ConstStr255Param name);
/*	Ï Establish a local volume or directory as a share point.
...
pascal	OSErr	GetUGEntry(short objType,
						   StringPtr objName,
						   long *objID);
/*	Ï Retrieve a user or group entry from the local file server.
