...
/*	Ï Initialize a FSSpec record.
	...
	Result Codes
		noErr				0		No error	
		nsvErr				-35		Volume doesnÍt exist	
...
pascal	OSErr	FSpGetFInfoCompat(const FSSpec *spec,
								  FInfo *fndrInfo);
/*	Ï Get the finder information for a file.
...
/*	Ï Set the finder information for a file.
...
pascal	OSErr	FSpCatMoveCompat(const FSSpec *source,
								 const FSSpec *dest);
/*	Ï Move a file or directory to a different location on on the same volume.
...
/*	Ï Open a file's resource file.
	...
	Result Codes
		noErr				0		No error
		nsvErr				Ü35		No such volume
		ioErr				Ü36		I/O error
		bdNamErr			Ü37		Bad filename or volume name (perhaps zero
									length)
		eofErr				Ü39		End of file
		tmfoErr				Ü42		Too many files open
		fnfErr				Ü43		File not found
		opWrErr				Ü49		File already open with write permission
		permErr				Ü54		Permissions error (on file open)
		extFSErr			Ü58		Volume belongs to an external file system
		memFullErr			Ü108	Not enough room in heap zone
		dirNFErr			Ü120	Directory not found
		mapReadErr			Ü199	Map inconsistent with operation
*/
...
/*	Ï Create a resource file.
	...
	Result Codes
		noErr				0		No error
		dirFulErr			Ü33		Directory full
		dskFulErr			Ü34		Disk full
		nsvErr				Ü35		No such volume
		ioErr				Ü36		I/O error
		bdNamErr			Ü37		Bad filename or volume name (perhaps zero
									length)
		tmfoErr				Ü42		Too many files open
		wPrErrw				Ü44		Disk is write-protected
		fLckdErr			Ü45		File is locked
*/
